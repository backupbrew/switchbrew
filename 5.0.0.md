...
downloading sooner than previously, even if the Nintendo Switch console
is in Sleep Mode
...
::\* When prompted to enter you Parental Controls PIN, press and hold
the + Button to change between input methods
...
::\* Play-Time Limit restrictions will still apply even when the
software title has been whitelisted.
...
`   return (FUSE_RESERVED_ODM4 & 0x800) &&  FUSE_RESERVED_ODM0 == 0x8E61ECAE &&  FUSE_RESERVED_ODM1  == 0xF2BA3BB2 ?  (FUSE_RESERVED_ODM2 & 0x1F) : 0`
  ...
  -   - Getconfig(17), which returns the Package2 hash if booting from
        recovery mode.
...
#### Kernel
  ...
  - 5 new syscalls.
      - Syscalls 0x48+0x49
        (svcAllocateUserHeapMemory+svcFreeUserHeapMemory) are used by
        [AM](AM%20services.md "wikilink")-sysmodule.
          - This is used to allocate Application pool.
      - Syscall 0x4A (svcSetUserHeapMemoryAllocationMax) is used by
        [PM](Process%20Manager%20services.md "wikilink")-sysmodule.
          - This allows PM to set a limit on the number of bytes AM can
            allocate from Application pool.
      - Syscall 0x6F (svcGetMemoryInfo) is used by FS, SM, and PM.
      - Syscall 0x2E (svcGetNextThreadInfo) was added, but it only works
        on dev units.
          - Allows fetching how many bytes free/occupied by the
            different pools.
      - svcControlCodeMemory was
        [updated](SVC#svcControlCodeMemory.md##svcControlCodeMemory "wikilink").
...
New fields were added to [System
Config](Settings%20services#System%20Config.md##System_Config "wikilink").
