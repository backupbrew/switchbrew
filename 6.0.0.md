...
### [nvservices-sysmodule](NV%20services.md "wikilink")

Among various changes, the ["Transfer Memory leak in nvservices system
module"](Switch%20System%20Flaws#System%20Modules.md##System_Modules "wikilink")
system flaw was patched in the following way:

  - [Initialize](NV%20services#Initialize.md##Initialize "wikilink") and
    [InitializeDevtools](NV%20services#InitializeDevtools.md##InitializeDevtools "wikilink")
    now keep track of the size of the transfer memory supplied by the
    user.
  - L\_34B90 (nvdrv's destructor) now calls L\_B4C30 (memset) on the
    entire transfer memory region (using the size saved previously)
    before calling
    [svcUnmapTransferMemory](SVC#svcUnmapTransferMemory.md##svcUnmapTransferMemory "wikilink").
...
<!-- end list -->
  ...
  - Redundant calls to smcGetConfig(12) have been reduced (12 calls -\>
    4 calls) during memory setup
...
##### [Loader](Loader%20services.md "wikilink")
  ...
  - A single null byte stack overflow due to strcat usage was fixed in
    content path parsing. See [Switch System
    Flaws](Switch%20System%20Flaws.md "wikilink") for details.
  - System Resource Size ([NPDM](NPDM.md "wikilink") +0x14) is now
    allowed to be non-zero for applets in addition to applications.

##### [PM](Process%20Manager%20services.md "wikilink")

Memory management initialization was greatly changed:

  - PM no longer hardcodes five memory profiles, and no longer calls
    smcGetConfig to determine which profile to use.
  - Instead, PM now uses
    [svcGetResourceLimitLimitValue](SVC.md "wikilink") to determine how
    much space the kernel has allotted for Application + Applet regions
    and [svcGetSystemInfo](SVC.md "wikilink") to determine the total
    physical memory available, and sets the System region size to (Total
    Memory Size - Application region size - Applet region size - 5 MiB).
